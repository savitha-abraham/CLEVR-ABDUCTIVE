...
class ClevrExecutor:
    ...
    def reason_U(self, constraint, ct_type, complete_scene, query_obj, region_seq, token_seq, outputs_seq):
    ...
    ##def run(self, x, index, split, guess=False, debug=False):
        ...
        if ans == 'error':
            flag_error = 1 
             ...
             #print("Ans is empty...")
             ans = self.reason_U(constraint, ct_type, complete_scene, query_obj, region_seq, token_seq, outputs_seq)
             flag_reason =1
             if ans == 'error':
               flag_re = 1 
        else:
...
#        self.modules['equal_size'] = self.equal_size
        ...
        #self.modules['addition'] = self.addition
        #self.modules['choose'] = self.choose
...
#        return 'error'
#    
#    def choose(self, scene, integer1):
#        if type(scene) == list and type(integer1) == int:
#            return integer1
#        return 'error'
#    
