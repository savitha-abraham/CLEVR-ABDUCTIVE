...
class Seq2seqParser(nn.Module):
    ...
    def reinforce_backward(self, entropy_factor=0.0):
        ...
        return self.seq2seq.reinforce_backward(self.reward, entropy_factor)

    def parse(self):
        #print("Self.x:", self.x, self.input_lengths)
        output_sequence = self.seq2seq.sample_output(self.x, self.input_lengths)
        #print("O/p Seq:", output_sequence)
        output_sequence = self._restore_order(output_sequence.data.cpu())
        #print("O/p Seq-1:", output_sequence)
    ...
    def _sort_batch(self, x, y):
        ...
        lengths_list = lengths_sorted.cpu().detach().numpy()
    ...
    def _restore_order(self, x):
        if self.idx_sorted is not None:
            ...
            inv_idxs = inv_idxs.to(device)
            self.idx_sorted = self.idx_sorted.to(device)
            x = x.to(device)
            inv_idxs.scatter_(0, self.idx_sorted, torch.arange(x.size(0)).long().to(device))
            return x[inv_idxs]
        return x
